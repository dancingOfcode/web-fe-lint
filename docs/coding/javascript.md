...
- 1.2.1.【强制】使用分号。`eslint`: [semi](https://eslint.org/docs/rules/semi)
  ...
  const shenzhen = {};
  const beijing = {}[(shenzhen, beijing)].forEach((jedi) => {
    jedi.father = 'vader';
  });
...
- 1.4.2.2.【参考】对于空代码块，可以将大括号直接闭合。
  ...
  ```javascript
  // bad
  if (condition) {} else if (otherCondition) {
  } else {
    // …
  }
...
- 1.7.1.【推荐】单行最大字符数：100。`eslint`: [max-len](https://eslint.org/docs/rules/max-len)
  ...
  ```javascript
  // bad
  const foo =jsonData && jsonData.foo && jsonData.foo.bar && jsonData.foo.bar.baz && jsonData.foo.bar.baz.quux && jsonData.foo.bar;
...
- 2.1.2.【强制】正确地使用 `const` 和 `let`。`eslint`: [prefer-const](https://eslint.org/docs/rules/prefer-const)
  ...
  声明时，应优先使用 `const`，只有当变量会被重新赋值时才使用 `let`：
