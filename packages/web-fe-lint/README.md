...
`web-fe-lint` 是[前端编码规范工程化](https://dancingofcode.github.io/web-fe-lint/)的配套 Lint 工具，可以为项目一键接入规范、一键扫描和修复规范问题，保障项目的编码规范和代码质量。
...
`web-fe-lint` 收敛屏蔽了这些依赖和配置细节，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡口，降低项目接入规范的成本。
...
```bash
npm install web-fe-lint -g
```

安装完成后，可执行 `web-fe-lint -h` 以验证安装成功。

### 使用

#### `web-fe-lint init`：一键接入

在项目根目录执行 `web-fe-lint init`，即可一键接入规范，为项目安装规范 `Lint` 所需的依赖和配置。

具体会做以下事情：

- 安装各种依赖：包括 `Linter` 依赖，如 [ESLint](https://eslint.org/)、[stylelint](https://stylelint.io/)、[commitlint](https://commitlint.js.org/#/)、[markdownlint](https://github.com/DavidAnson/markdownlint) 等；配置依赖，如 [wen-fe-eslint](https://www.npmjs.com/package/eslint-config-fe)、[web-fe-stylelint](https://www.npmjs.com/package/web-fe-stylelint)、[web-fe-commitlint](https://www.npmjs.com/package/web-fe-commitlint)、[web-fe-markdownlint](https://www.npmjs.com/package/web-fe-markdownlint) 等
- 写入各种配置文件，包括：
  - `.eslintrc.js`、`.eslintignore`：ESLint 配置（继承 `eslint-config-fe`）及黑名单文件
  ...
  - `web-fe-lint.config.js`web-fe-lint 包的一些配置，如启用的功能等
- 配置 git commit 卡口：使用 [husky](https://www.npmjs.com/package/husky) 设置代码提交卡口，在 git commit 时会运行 `web-fe-lint commit-file-scan` 和 `web-fe-lint commit-msg-scan` 分别对提交文件和提交信息进行规范检查。`web-fe-lint commit-file-scan` 默认仅对 error 问题卡口，如果你想对 warn 问题也卡口，可以增加 `--strict` 参数以开启严格模式

> 注 1：如果项目已经配置过 ESLint、stylelint 等 Linter，执行 `web-fe-lint init` 将会提示存在冲突的依赖和配置，并在得到确认后进行覆盖：
>
> 注 2：如果项目的 .vscode/ 目录被 .gitignore 忽略，可以在拉取项目后单独执行 `web-fe-lint init --vscode` 命令写入 `.vscode/extensions.json` 和 `.vscode/settings.json` 配置文件

#### `web-fe-lint scan`：一键扫描
...
> 注 1：事实上，你可以在任意目录执行 `web-fe-lint scan` `web-fe-lint` 会根据文件类型、JSON 等特征嗅探项目类型。但我们还是推荐在执行过 `web-fe-lint init` 的项目根目录执行 `web-fe-lint scan`，以得到最准确的扫描结果。
>
> 注 2: `web-fe-lint` 会根据项目内有无 eslint 和 stylelint 配置文件判断使用项目的配置文件还是 `web-fe-lint` 默认配置进行扫描。若使用项目的，在未安装依赖时会帮其安装（执行 npm i）。若使用项目配置扫描失败，则使用默认配置扫描

#### `web-fe-lint fix`：一键修复
...
#### `web-fe-lint commit-file-scan` 提交文件扫描
...
#### `web-fe-lint commit-msg-scan` 提交信息扫描
...
- web-fe-lint.init(config)：将项目一键接入规范，效果等同于 `web-fe-lint init`
...
| ------------------ | ---------- | ------ | ------------------------------------------------------------------------------------------------------------------- |
| cwd                | string     | -      | 项目绝对路径                                                                                                        |
| eslintType         | ESLintType | -      | 语言和框架类型，如果不配置，等同于 web-fe-lint init，控制台会出现选择器，如果配置，控制台就不会出现选择器        |
| enableESLint       | boolean    | true   | 是否启用 ESLint，如果不配置默认值为 true，即默认启用 ESLint                                                         |
| enableStylelint    | boolean    | -      | 是否启用 stylelint，如果不配置，等同于 web-fe-lint init，控制台会出现选择器，如果配置，控制台就不会出现选择器    |
| enableMarkdownlint | boolean    | -      | 是否启用 markdownlint，如果不配置，等同于 web-fe-lint init，控制台会出现选择器，如果配置，控制台就不会出现选择器 |
...
`web-fe-lint` 基于一份配置进行扫描（但你也可以零配置使用），支持的配置参数有：
...
`web-fe-lint` 会读取执行目录下的 `web-fe-lint.config.js` 作为配置文件。`web-fe-lint init` 会在执行目录下新增如下的 `web-fe-lint.config.js` 文件：
...
如果你的 TS 项目 commit 卡口和 `web-fe-lint scan` 运行时间很长，可以通过如下在 `.eslintrc.js` 中增加以下配置提升性能：
